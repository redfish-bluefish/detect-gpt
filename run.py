...
rewriting_model_name = "MBZUAI/LaMini-Flan-T5-783M"
rewriting_tokenizer = transformers.AutoTokenizer.from_pretrained(rewriting_model_name)
rewriting_model = transformers.AutoModelForSeq2SeqLM.from_pretrained(
...
def get_lls(texts):
    if not args.openai_model:
        ...
        API_TOKEN_COUNTER += (
            total_tokens * 2
        )  # multiply by two because OpenAI double-counts echo_prompt tokens
...
def get_roc_metrics(real_preds, sample_preds):
    ...
    real_preds = [x if not np.isnan(x) else np.float64(0) for x in real_preds]
    sample_preds = [x if not np.isnan(x) else np.float64(0) for x in sample_preds]
    fpr, tpr, _ = roc_curve(
...
def save_ll_histograms(experiments):
    ...
    for experiment in experiments:
        try:
            ...
            plt.subplot(1, 2, 2)
            plt.hist(
                [r["original_ll"] for r in results],
                alpha=0.5,
                bins="auto",
                label="original",
            )
...
def save_llr_histograms(experiments):
    ...
    for experiment in experiments:
        try:
            ...
            plt.hist(
                [r["sampled_llr"] for r in results],
                alpha=0.5,
                bins="auto",
                label="sampled",
            )
            plt.hist(
                [r["original_llr"] for r in results],
                alpha=0.5,
                bins="auto",
                label="original",
            )
...
# Custom perturbation functions (ADDED)
def perturb_texts_reorder_sentences(texts, span_length, pct, ceil_pct=False):
    def reorder_sentences(text):
        doc = nlp(text)
        sentences = [str(sent) for sent in doc.sents]
        reordered_text = list(np.random.permutation(sentences))

        return " ".join(reordered_text)

    nlp = spacy.load("en_core_web_sm")

    perturbed_texts = list(map(reorder_sentences, texts))
    return perturbed_texts


def perturb_texts_rewrite(texts, span_length, pct, ceil_pct=False):
    def rewrite_text(text):
        return f"Please rewrite the following text: {text}"

    chunk_size = args.chunk_size
    outputs = []
    for i in tqdm.tqdm(range(0, len(texts), chunk_size), desc="Applying perturbations"):
        prompts = list(map(rewrite_text, texts[i : i + chunk_size]))
        inputs = rewriting_tokenizer(prompts, return_tensors="pt", padding=True).to(
            DEVICE
        )
        generations = rewriting_model.generate(**inputs, max_length=512, do_sample=True)
        perturbed_texts = rewriting_tokenizer.batch_decode(
...
def perturb_texts_deletion(texts, span_length, pct, ceil_pct=False):
    masked_texts = [tokenize_and_mask(x, 1, pct, ceil_pct) for x in texts]
    perturbed_texts = masked_texts
    # replace each <extra_id_*> with nothing, to effectively cause deletion
    for idx, text in enumerate(perturbed_texts):
        filled_text = text
        for fill_idx in range(count_masks([text])[0]):
            filled_text = filled_text.replace(f"<extra_id_{fill_idx}>", "")
...
def get_perturbation_results(
    ...
    n_perturbations=1,
    n_samples=500,
    perturbation_function=perturb_texts,
):
    perturb_fn = functools.partial(
        perturbation_function, span_length=span_length, pct=args.pct_words_masked
    )

    return get_perturbation_results_(
        perturb_fn,
...
def get_perturbation_results_(
    perturb_fn, span_length=10, n_perturbations=1, n_samples=500
):
    load_mask_model()
...
def run_perturbation_experiment(
    results, criterion, span_length=10, n_perturbations=1, n_samples=500, name=None
):
...
def generate_samples(raw_data, batch_size):
    ...
    for batch in range(len(raw_data) // batch_size):
        ...
        sampled_text = sample_from_model(
            original_text, min_words=30 if args.dataset in ["pubmed"] else 55
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip_poor_performers", action="store_true")
    ...
    if not args.skip_baselines:
        baseline_outputs = [
            run_baseline_threshold_experiment(get_ll, "likelihood", n_samples=n_samples)
        ]
        if args.openai_model is None:
            rank_criterion = lambda text: -get_rank(text, log=False)
            baseline_outputs.append(
                run_baseline_threshold_experiment(
                    rank_criterion, "rank", n_samples=n_samples
                )
            )
            logrank_criterion = lambda text: -get_rank(text, log=True)
            baseline_outputs.append(
                run_baseline_threshold_experiment(
                    logrank_criterion, "log_rank", n_samples=n_samples
                )
            )
            entropy_criterion = lambda text: get_entropy(text)
            baseline_outputs.append(
                run_baseline_threshold_experiment(
                    entropy_criterion, "entropy", n_samples=n_samples
                )
            )
    ...
    outputs = []
    max_n_perturbations = max(n_perturbation_list)

    if not args.skip_poor_performers:
        for perturbation_mode in ["z"]:
            # Run model rewriting perturbation experiment
            perturbation_results = get_perturbation_results(
                perturbation_function=perturb_texts_rewrite,
                n_perturbations=max_n_perturbations,
            )
            output = run_perturbation_experiment(
                ...
                n_perturbations=max_n_perturbations,
                n_samples=n_samples,
                name=f"perturbation_model_rewrite_{max_n_perturbations}",
            )
            outputs.append(output)
            with open(
                os.path.join(
                    SAVE_FOLDER,
                    f"perturbation_model_rewrite_{perturbation_mode}_results.json",
                ),
                "w",
            ) as f:
                json.dump(output, f)
        ...
        for perturbation_mode in ["z"]:
            # Run text deletion perturbation experiment
            perturbation_results = get_perturbation_results(
                perturbation_function=perturb_texts_deletion,
                n_perturbations=max_n_perturbations,
            )
            output = run_perturbation_experiment(
                perturbation_results,
                ...
                n_perturbations=max_n_perturbations,
                n_samples=n_samples,
                name=f"perturbation_text_deletion_{max_n_perturbations}",
            )
            outputs.append(output)
            with open(
                os.path.join(
                    SAVE_FOLDER,
                    f"perturbation_text_deletion_{perturbation_mode}_results.json",
                ),
                "w",
            ) as f:
                json.dump(output, f)
    ...
    for perturbation_mode in ["z"]:
        # Run sentence reordering perturbation experiment
        perturbation_results = get_perturbation_results(
            perturbation_function=perturb_texts_reorder_sentences,
            n_perturbations=max_n_perturbations,
        )
        output = run_perturbation_experiment(
            perturbation_results,
            ...
            n_perturbations=max_n_perturbations,
            n_samples=n_samples,
            name=f"perturbation_sentence_reordering_{max_n_perturbations}",
        )
        outputs.append(output)
        with open(
            os.path.join(
                SAVE_FOLDER,
                f"perturbation_sentence_reordering_{perturbation_mode}_results.json",
            ),
            "w",
        ) as f:
            json.dump(output, f)
    ...
    if not args.baselines_only:
        ...
        for n_perturbations in n_perturbation_list:
            perturbation_results = get_perturbation_results(
                args.span_length, n_perturbations, n_samples
            )
            for perturbation_mode in ["z"]:
                output = run_perturbation_experiment(
                    perturbation_results,
    ...
    if not args.skip_baselines:
        ...
        with open(
            os.path.join(SAVE_FOLDER, f"likelihood_threshold_results.json"), "w"
        ) as f:
        ...
        if args.openai_model is None:
            ...
            with open(
                os.path.join(SAVE_FOLDER, f"rank_threshold_results.json"), "w"
            ) as f:
                json.dump(baseline_outputs[1], f)

            # write log rank threshold results to a file
            with open(
                os.path.join(SAVE_FOLDER, f"logrank_threshold_results.json"), "w"
            ) as f:
                json.dump(baseline_outputs[2], f)

            # write entropy threshold results to a file
            with open(
                os.path.join(SAVE_FOLDER, f"entropy_threshold_results.json"), "w"
            ) as f:
                json.dump(baseline_outputs[3], f)

        # write supervised results to a file
        with open(
            os.path.join(SAVE_FOLDER, f"roberta-base-openai-detector_results.json"), "w"
        ) as f:
            json.dump(baseline_outputs[-2], f)

        # write supervised results to a file
        with open(
            os.path.join(SAVE_FOLDER, f"roberta-large-openai-detector_results.json"),
            "w",
        ) as f:
            json.dump(baseline_outputs[-1], f)
